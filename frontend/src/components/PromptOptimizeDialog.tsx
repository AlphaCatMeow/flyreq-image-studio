      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-muted-foreground">{t('promptOptimizeDialog.original')}</span>
            <div className="min-h-[120px] max-h-[300px] overflow-y-auto rounded-lg border border-border bg-muted/40 px-3 py-2.5 text-sm leading-relaxed text-foreground/80 whitespace-pre-wrap">
              {originalPrompt || <span className="text-muted-foreground italic">{t('promptOptimizeDialog.empty')}</span>}
            </div>
          </div>

          {/* 优化后提示词 */}
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-primary">{t('promptOptimizeDialog.optimized')}</span>
            <div className={cn(
              ...
              {loading && !optimizedPrompt ? (
                <span className="flex items-center gap-2 text-muted-foreground">
                  ...
                  {t('promptOptimizeDialog.optimizing')}
                </span>
              ) : optimizedPrompt ? (
                <>
                  {optimizedPrompt}
                ...
                <span className="text-muted-foreground italic">{t('promptOptimizeDialog.waiting')}</span>
              )}
            </div>
          </div>
        </div>
